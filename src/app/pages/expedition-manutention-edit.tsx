...
export default function EditExpeditionManutention() {
  const [content, setContent] = useState("");
  const [name, setName] = useState("Expedition et manutention");
  const [slug, setSlug] = useState("expedition-manutention");
  const [model, setModel] = useState("default");
  const router = useRouter();

  useEffect(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("expedition-manutention-content") : null;
    if (saved) setTimeout(() => setContent(saved), 0);
  }, []);

  const handleSave = () => {
    if (typeof window !== "undefined") {
      localStorage.setItem("expedition-manutention-content", content);
    }
    router.push("/pages/expedition-manutention");
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-white rounded shadow mt-8">
      ...
      <div className="mb-6">
        ...
        <textarea className="w-full border rounded px-3 py-2 min-h-[200px] font-mono input-placeholder-normal"
          value={content}
      ...
      <div className="flex gap-4 justify-end">
        ...
        <button
          ...
          onClick={() => router.push("/pages/expedition-manutention")}
        >
          Annuler
        </button>
      </div>

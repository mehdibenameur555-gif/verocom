...
export default function EditMethodesPaiement() {
  const [content, setContent] = useState("");
  const [name, setName] = useState("Méthodes de payement");
  const [slug, setSlug] = useState("methodes-paiement");
  ...
  const handleSave = () => {
    if (typeof window !== "undefined") {
      localStorage.setItem("methodes-paiement-content", content);
    }
    router.push("/pages/methodes-paiement");
  };
    ...
    <div className="max-w-3xl mx-auto p-8 bg-white rounded shadow mt-8">
      ...
      <div className="flex gap-4 justify-end">
        ...
        <button
          ...
          onClick={() => router.push("/pages/methodes-paiement")}
        >
          Annuler
        </button>
      </div>
